...
namespace practicaUWP
{
    ...
    public sealed partial class DeletePage : Page
    {
        public DeletePage()
        {
            ...
            listadoAlumnos.ItemsSource = App.ListaAlumnos;
        }
        ...
        async private void Click_Delete(object sender, RoutedEventArgs e)
        {
            bool encontrado = false;
            int index = -1;
            foreach (Alumno a in App.ListaAlumnos)
            {
                if (a.Dni.Equals(cajaEliminar.Text))
                {
                    encontrado = true;
                    index = App.ListaAlumnos.IndexOf(a);
                }
            }
            ...
            if (index >= 0 && index <= App.ListaAlumnos.Count-1)
            {
                App.ListaAlumnos.RemoveAt(index);
            }

            List<Alumno> lista = new List<Alumno>(App.ListaAlumnos);
            listadoAlumnos.ItemsSource = lista;
            limpiar();
        }
        private void limpiar()
        {
            cajaEliminar.Text = string.Empty;
        }
